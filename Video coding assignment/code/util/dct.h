...
inline float mat_get_Element(const std::vector<float>& mat, int x, int y){
    return mat[y*Block::width + x];
}

inline void mat_set_Element(std::vector<float>& mat, int x, int y, float value){
    mat[y*Block::width + x] = value;
}

inline std::vector<float> matrix_mult(std::vector<float> b1, std::vector<float> b2) {
    std::vector<float> b{};
...
inline Block forward_dct(Block block){
    // Y = A X At
    std::vector<float> X;
...
inline Block inverse_dct(Block block){
    // X = At Y A
    std::vector<float> Y;
    for (int i = 0; i < block.data.size();++i){
